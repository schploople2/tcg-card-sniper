...
export function useAllListings() {
  return useQuery({
    ...
    queryFn: async (): Promise<(Listing & { card: { id: string; cardName: string; setName: string; cardNumber: string | null; condition: string; targetPrice: number } })[]> => {
      const { data } = await api.get<{ listings: (Listing & { card: { id: string; cardName: string; setName: string; cardNumber: string | null; condition: string; targetPrice: number } })[] }>("/api/listings");
