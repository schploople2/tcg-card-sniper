...
| `ssw` — "Validation error" on Save analysis blocked the entire pick-printing feature | Server's `addedCardSchema.note` was `.optional()` but client always sends `note: null`; Zod rejects null. Generic toast hid the field. | Schema → `.nullable().optional()` (commit 87e5c55). Also: error handler now puts the failing field+message into the top-level `error` so toasts surface the real cause. Verified live: "Analysis saved" toast appears. |
| `rys` — OCR returned "No cards identified" on EVERY cached lot (e.g. ebay v1\|147321331349\|0) | `visionOneImage` writes the coerced shape `{name, ...}` to `LotImage.ocrText`, but the cache-hit replay ran each entry through `coerceSuggestion()` which reads `raw.cardName` → undefined → returns null. Every cached suggestion silently dropped. Existing test mocked the wrong shape (`cardName`) so the bug never tripped CI. | New `parseCachedSuggestions()` helper that parses stored JSON directly without re-coercing (accepts `name` or `cardName` defensively). Updated existing cache test to use production shape + added byte-for-byte regression test using real prod cache string. Commit 6a42034. Verified live: same lot now shows 30+ chips. |
